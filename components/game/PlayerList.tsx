...
// Contenu du menu « Royaume » de la top bar : le classement des fiefs par
// prestige. Cliquer un joueur charge son fief (le menu se referme via la
// render-prop de TopBarMenu). Le changement de nom vit désormais dans le menu
// « Mon fief ».
export function PlayerList({
  ...
  return (
    <div className="flex max-h-[70vh] w-64 flex-col">
      <div className="border-b border-gold/30 px-4 py-3">
        <h2 className="font-serif text-lg leading-none text-ink">
          Fiefs du royaume
        </h2>
        <p className="mt-1 font-sans text-[10px] uppercase tracking-widest text-ink/60">
      ...
      <ul className="flex-1 overflow-y-auto px-2 py-2">
        {players.length === 0 ? (
          <li className="px-2 py-3 font-serif text-xs italic text-ink/50">
          ...
          players.map((player, index) => {
            ...
            return (
              <li key={player.playerId}>
                ...
                >
                  <span className="w-4 shrink-0 text-right font-sans text-[10px] tabular-nums text-ink/50">
                    {index + 1}
                  </span>
                  {isOwn ? (
                  ...
                  <span
                    className={`flex-1 truncate font-serif text-base ${
                      player.name ? "text-ink" : "italic text-ink/60"
                    }`}
                  >
                    {label}
                  </span>
                  ...
                  >
                    <ResourceIcon kind="prestige" className="pixelated h-3 w-3" />
